...
const RuntimeSetup = ({ onRuntimeReady }) => {
  ...
  const handleStartRuntime = async (runtimeName) => {
    try {
      ...
      const result = await startRuntime(runtimeName);
      
      // Check for both result.success (boolean) and result.status === 'success' (string)
      if (result.success || result.status === 'success') {
        ...
        if (result.requiresManualAction || result.requires_manual_action) {
  ...
  const handleSetCustomDockerPath = async () => {
    try {
      setPathError('');
      setCustomDockerPath(customDockerPath);
  ...
  const handleSetCustomPodmanPath = async () => {
    try {
      setPathError('');
      setCustomPodmanPath(customPodmanPath);
      // Refresh runtime status after setting custom path
      await checkRuntimeStatus();
    } catch (err) {
          ...
          <div className="empty-state-container">
            <div className="empty-state-content">
              ...
              <p className="empty-state-description">
                Using {runtimeStatus.preferredRuntime} - insta-infra is ready!
              </p>
              {onRuntimeReady && (
                <button onClick={onRuntimeReady} className="button button-primary" style={{ marginTop: '1rem' }}>
                  <svg width="16" height="16" className="button-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
